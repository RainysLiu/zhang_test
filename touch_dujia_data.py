...
# 1、获取出发地点信息(自由行)
response = requests.get("https://touch.dujia.qunar.com/depCities.qunar")
    ...
    for dep in dep_dict["data"][dep_item]:
        ...
        for item in a:
            ...
            url = "https://touch.dujia.qunar.com/list?modules=list," \
                  "bookingInfo,activityDetail&dep={0}&query={1}" \
                  "&dappDealTrace=true&mobFunction=%E6%89%A9%E5%B1%95%E8%" \
                  "87%AA%E7%94%B1%E8%A1%&cfrom=zyx&it=dujia_hy_destination" \
                  "&page=home&date=&needNoResult=true&originalquery={2}&" \
                  "limit=0,24&includeAD=true&qsact=search".format(
